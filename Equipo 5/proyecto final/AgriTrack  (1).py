...
PERMISOS_POR_ROL = {
    ROLES['ADMIN']: ['ver', 'editar', 'agregar', 'eliminar', 'transferir', 'gestion_usuarios', 'gestion_categorias'],
    ROLES['USER']: ['ver', 'editar', 'agregar', 'eliminar', 'transferir'],
    ROLES['VIEWER']: ['ver']
}

ARCHIVOS = {
    'usuarios': "usuarios_sistema.json.enc",
    'clave_maestra': "clave_maestra.key.enc",
    'datos': "agricultura_finanzas.json.enc",
    'presupuesto_mensual': "agricultura_presupuesto_mensual.json.enc",
    'proveedores': "agricultura_proveedores.json.enc",
    'presupuestos_anuales': "agricultura_presupuestos_anuales.json.enc",
    'sobrantes_anuales': "agricultura_sobrantes_anuales.json.enc",
    'categorias_personalizadas': "agricultura_categorias_personalizadas.json.enc",
    'log_actividades': "log_actividades.json"
}

class SistemaSeguridad:
    def __init__(self):
        self.archivo_usuarios = ARCHIVOS['usuarios']
        self.archivo_clave_maestra = ARCHIVOS['clave_maestra']
        self.usuarios = {}
        self.max_intentos = 3
        self.cipher = None
        self.inicializar_cifrado()
        self.cargar_usuarios()
    
    def inicializar_cifrado(self):
        """Inicializa el sistema de cifrado con una clave maestra"""
        try:
            Path("data").mkdir(exist_ok=True)
            
            key_file = self.archivo_clave_maestra
            key = None
            
            master_password = "AgriTrackMasterKey2024!"
            salt = b'AgriTrackSalt2024'
            
            kdf = hashlib.pbkdf2_hmac('sha256', master_password.encode(), salt, 100000)
            cipher_master = Fernet(base64.urlsafe_b64encode(kdf))
            
            if os.path.exists(key_file):
                with open(key_file, 'rb') as f:
                    encrypted_key = f.read()
                key = cipher_master.decrypt(encrypted_key)
            else:
                key = Fernet.generate_key()
                encrypted_key = cipher_master.encrypt(key)
            ...
            messagebox.showerror("Error Crítico", 
                f"Error al inicializar el sistema de cifrado: {str(e)}\n"
                "La aplicación no puede continuar.")
            raise e
    
    def cifrar_datos(self, datos):
        """Cifra datos para guardarlos en disco"""
        try:
            if self.cipher:
    ...
    def descifrar_datos(self, datos_cifrados):
        """Descifra datos desde el disco"""
        try:
            if self.cipher and datos_cifrados:
    ...
    def guardar_archivo_cifrado(self, archivo, datos):
        """Guarda un archivo con datos cifrados"""
        try:
            datos_cifrados = self.cifrar_datos(datos)
            if datos_cifrados:
                with open(f"data/{archivo}", 'wb') as f:
    ...
    def cargar_archivo_cifrado(self, archivo, valor_por_defecto=None):
        """Carga un archivo con datos cifrados"""
        try:
            ruta = f"data/{archivo}"
            if os.path.exists(ruta):
                with open(ruta, 'rb') as f:
                    datos_cifrados = f.read()
                datos = self.descifrar_datos(datos_cifrados)
    ...
    def validar_fortaleza_password(self, password):
    ...
    def cargar_usuarios(self):
        datos = self.cargar_archivo_cifrado(self.archivo_usuarios)
        if datos:
            self.usuarios = datos
        else:
    ...
    def guardar_usuarios(self):
        self.guardar_archivo_cifrado(self.archivo_usuarios, self.usuarios)
    ...
    def crear_usuario(self, username, nombre_completo, password, rol, pregunta_seguridad, respuesta_seguridad):
        if username in self.usuarios:
            return False, "El usuario ya existe"
        
        valida, mensaje = self.validar_fortaleza_password(password)
        if not valida:
            return False, mensaje
        
        salt, pwd_hash = self.hash_password(password)
        salt_resp, resp_hash = self.hash_password(respuesta_seguridad.strip().lower())
    ...
    def restablecer_password(self, username, nueva_password):
        ...
        valida, mensaje = self.validar_fortaleza_password(nueva_password)
        if not valida:
            return False, mensaje
...
class VentanaLogin:
    ...
    def crear_interfaz(self):
        ...
        tk.Label(frame_principal, text="Sistema de Gestión Agrícola", font=('Arial', 12), bg='#2d5016', fg='#90EE90').pack(pady=(0, 20))
    ...
    def abrir_recuperacion(self):
        ...
        lbl_pregunta = tk.Label(frame_pasos_siguientes, text="", font=('Arial', 10, 'italic'), bg='white', wraplength=350)
        
        frame_respuesta = tk.Frame(frame_pasos_siguientes, bg='white')
        frame_respuesta.pack(fill='x', pady=5)
        
        mostrar_resp_var = tk.BooleanVar()
        entry_respuesta = ttk.Entry(frame_respuesta, width=30, show='●')
        entry_respuesta.pack(side='left', fill='x', expand=True)
        
        tk.Checkbutton(
            frame_respuesta, 
            text="👁️", 
            variable=mostrar_resp_var,
            command=lambda: entry_respuesta.config(show='' if mostrar_resp_var.get() else '●'),
            bg='white'
        ).pack(side='right', padx=5)
        
        frame_pass1 = tk.Frame(frame_pasos_siguientes, bg='white')
        frame_pass1.pack(fill='x', pady=5)
        entry_nueva_pass = ttk.Entry(frame_pass1, width=30, show='●')
        entry_nueva_pass.pack(side='left', fill='x', expand=True)
        
        mostrar_pass1_var = tk.BooleanVar()
        tk.Checkbutton(
            frame_pass1, 
            text="👁️", 
            variable=mostrar_pass1_var,
            command=lambda: entry_nueva_pass.config(show='' if mostrar_pass1_var.get() else '●'),
            bg='white'
        ).pack(side='right', padx=5)
        
        frame_pass2 = tk.Frame(frame_pasos_siguientes, bg='white')
        frame_pass2.pack(fill='x', pady=5)
        entry_conf_pass = ttk.Entry(frame_pass2, width=30, show='●')
        entry_conf_pass.pack(side='left', fill='x', expand=True)
        
        mostrar_pass2_var = tk.BooleanVar()
        tk.Checkbutton(
            frame_pass2, 
            text="👁️", 
            variable=mostrar_pass2_var,
            command=lambda: entry_conf_pass.config(show='' if mostrar_pass2_var.get() else '●'),
            bg='white'
        ).pack(side='right', padx=5)
        ...
        def buscar_usuario():
            ...
            if user in self.seguridad.usuarios:
                ...
                if 'pregunta_seguridad' in datos_usuario and datos_usuario['pregunta_seguridad']:
                    ...
                    tk.Label(frame_pasos_siguientes, text="2. Respuesta de seguridad:", 
                            font=('Arial', 10, 'bold'), bg='white').pack(anchor='w', pady=(10, 0))
                    lbl_pregunta.pack(anchor='w', pady=5)
                    frame_respuesta.pack(fill='x')
                    
                    tk.Label(frame_pasos_siguientes, text="3. Nueva Contraseña:", 
                            font=('Arial', 10, 'bold'), bg='white').pack(anchor='w', pady=(15, 0))
                    frame_pass1.pack(fill='x')
                    
                    tk.Label(frame_pasos_siguientes, text="Confirmar Contraseña:", 
                            font=('Arial', 10, 'bold'), bg='white').pack(anchor='w')
                    frame_pass2.pack(fill='x')
...
class SistemaFinancieroAgricolaSeguro:
    ...
    def validar_fecha(self, fecha_str):
        """Valida que una fecha sea correcta y devuelve objeto datetime o None"""
        try:
    ...
    def registrar_actividad(self, actividad, detalles=""):
        ...
        if len(self.log_actividades) > 10000:
            self.log_actividades = self.log_actividades[-10000:]
        
        self.guardar_log_actividades()
    
    def guardar_log_actividades(self):
        try:
            with open(ARCHIVOS['log_actividades'], 'w', encoding='utf-8') as f:
                json.dump(self.log_actividades, f, ensure_ascii=False, indent=2)
        except:
            pass
    
    def cargar_log_actividades(self):
        if os.path.exists(ARCHIVOS['log_actividades']):
            try:
                with open(ARCHIVOS['log_actividades'], 'r', encoding='utf-8') as f:
    ...
    def verificar_permiso(self, accion):
        ...
        return self.seguridad.verificar_permiso(self.usuario_actual, accion)
    
    def puede_modificar(self):
        """Verifica si el usuario puede modificar datos"""
        return self.verificar_permiso('editar')
    
    def puede_eliminar(self):
        """Verifica si el usuario puede eliminar datos"""
        return self.verificar_permiso('eliminar')
    
    def puede_gestionar_usuarios(self):
        """Verifica si el usuario puede gestionar otros usuarios"""
        return self.verificar_permiso('gestion_usuarios')
    ...
    def crear_pestañas_segun_rol(self):
        if self.rol_usuario == ROLES['ADMIN']:
    ...
    def crear_tab_usuarios(self):
        if not self.puede_gestionar_usuarios():
            return
        ...
        self.combo_nuevo_rol = ttk.Combobox(frame_campos, values=[ROLES['ADMIN'], ROLES['USER'], ROLES['VIEWER']], state='readonly', width=20)
    ...
    def cambiar_contraseña(self):
        ...
        ventana.geometry(f'400x450+{x}+{y}')
        ...
        lbl_requisitos = tk.Label(
            frame, 
            text="Requisitos: Mínimo 8 caracteres, 1 mayúscula, 1 minúscula, 1 número",
            font=('Arial', 8),
            fg='#666'
        )
        lbl_requisitos.pack(pady=(0, 10))
    ...
    def calcular_sobrante_año(self, año):
        ...
        total_gastado = sum(t['monto'] for t in self.transacciones if self.validar_fecha(t['fecha']) and self.validar_fecha(t['fecha']).year == año)
    ...
    def distribuir_sobrante_personalizado(self, sobrante, año_anterior):
        ...
        def aplicar_distribucion():
            ...
            if abs(total - 100) > 0.01: 
                messagebox.showerror("Error", f"Los porcentajes deben sumar 100%\nActual: {total:.1f}%")
                return
    ...
    def actualizar_monto_distribucion(self, entry, label, sobrante_total):
        try: 
            ...
            label.config(text=f"${sobrante_total * (valor / 100):,.2f}")
        except: 
    ...
    def actualizar_info_año(self):
        ...
        if hasattr(self, 'tree_historial_años'):
            ...
            for año in sorted(self.presupuestos_por_año.keys(), reverse=True):
                ...
                gastado = sum(t['monto'] for t in self.transacciones if self.validar_fecha(t['fecha']) and self.validar_fecha(t['fecha']).year == int(año))
    ...
    def crear_tab_categorias_custom(self):
        if not self.puede_modificar():
            messagebox.showinfo("Información", "No tiene permisos para gestionar categorías")
            return
    ...
    def agregar_nueva_categoria(self):
        ...
        self.registrar_actividad("Crear categoría", f"Categoría: {nombre}")
    ...
    def agregar_nueva_subcategoria(self):
        ...
        if categoria in self.categorias_agricolas:
            subcats_existentes.extend(self.categorias_agricolas[categoria])
        if categoria in self.categorias_personalizadas:
            subcats_existentes.extend(self.categorias_personalizadas[categoria])
        
        if subcategoria in subcats_existentes:
            messagebox.showwarning("Advertencia", "Ya existe esa subcategoría")
            return
        
        if categoria not in self.categorias_personalizadas:
            self.categorias_personalizadas[categoria] = []
        ...
        self.actualizar_vista_categorias()
        self.actualizar_todas_categorias_combos()
        self.registrar_actividad("Crear subcategoría", f"{categoria} - {subcategoria}")
    ...
    def eliminar_categoria(self):
        ...
        if categoria in self.categorias_agricolas and categoria not in self.categorias_personalizadas:
            messagebox.showwarning("Advertencia", "No se pueden eliminar categorías predefinidas")
            return
        ...
        if messagebox.askyesno("Confirmar", f"¿Eliminar la categoría '{categoria}'?\n\nSe eliminarán también todas sus subcategorías."):
            ...
            self.actualizar_vista_categorias()
            self.actualizar_todas_categorias_combos()
            self.registrar_actividad("Eliminar categoría", f"Categoría: {categoria}")
    ...
    def eliminar_subcategoria(self):
        ...
        if messagebox.askyesno("Confirmar", f"¿Eliminar la subcategoría '{subcategoria}' de '{categoria}'?"):
  
            if categoria not in self.categorias_personalizadas:
                self.categorias_personalizadas[categoria] = list(self.categorias_agricolas.get(categoria, []))
            ...
            if subcategoria in self.categorias_personalizadas[categoria]:
                ...
                self.actualizar_vista_categorias()
                self.actualizar_todas_categorias_combos()
                self.registrar_actividad("Eliminar subcategoría", f"{categoria} - {subcategoria}")
    ...
    def actualizar_subcats_eliminar(self, event=None):
        ...
        subcats = []
        if categoria in self.categorias_agricolas:
            subcats.extend(self.categorias_agricolas[categoria])
        if categoria in self.categorias_personalizadas:
            subcats.extend(self.categorias_personalizadas[categoria])
        
        subcats_unicos = []
        for sub in subcats:
            if sub not in subcats_unicos:
                subcats_unicos.append(sub)
        
        self.combo_subcat_eliminar['values'] = subcats_unicos
        if subcats_unicos: 
            self.combo_subcat_eliminar.current(0)

    def actualizar_vista_categorias(self):
        if not hasattr(self, 'text_categorias'):
            return
        ...
        for categoria in sorted(todas_categorias.keys()):
            tipo = ""
            if categoria in self.categorias_agricolas and categoria in self.categorias_personalizadas:
                tipo = " [MIXTA]"
            ...
            texto += f"\n{categoria}{tipo}\n" + "-" * 100 + "\n"
            subcats = sorted(todas_categorias[categoria])
            if subcats:
        ...
        if lista_categorias:
            if hasattr(self, 'combo_cat_para_subcat') and not self.combo_cat_para_subcat.get():
                self.combo_cat_para_subcat.current(0)
            if hasattr(self, 'combo_cat_eliminar') and not self.combo_cat_eliminar.get():
    ...
    def regenerar_presupuesto_categorias(self):
        if not hasattr(self, 'frame_categorias_presupuesto'):
            return
        ...
        for row, categoria in enumerate(sorted(todas_categorias.keys())):
            ...
            if categoria in self.categorias_personalizadas and categoria not in self.categorias_agricolas:
                tipo = " [PERSONALIZADA]"
            elif categoria in self.categorias_agricolas and categoria in self.categorias_personalizadas:
                tipo = " [MIXTA]"
    ...
    def cargar_presupuesto_mes(self, event=None):
        if not hasattr(self, 'presupuesto_mensual_entries'):
            return
        ...
        if mes in self.presupuesto_mensual_por_mes:
            ...
            for categoria, entry in self.presupuesto_mensual_entries.items():
                ...
                if self.puede_modificar() and hasattr(self, 'presupuesto_mensual_labels_modificado'):
                    if mes in self.presupuesto_modificado and categoria in self.presupuesto_modificado.get(mes, {}) and self.presupuesto_modificado[mes].get(categoria, False):
    ...
    def guardar_presupuesto_mes(self):
        ...
        for categoria, entry in self.presupuesto_mensual_entries.items():
            try:
                ...
                if monto < 0:
                    messagebox.showerror("Error", f"El monto en '{categoria}' no puede ser negativo")
                    return
                self.presupuesto_mensual_por_mes[mes][categoria] = monto
            except ValueError:
                messagebox.showerror("Error", f"Monto inválido en '{categoria}'. Debe ser un número.")
                return
    ...
    def modificar_presupuesto_categoria(self, categoria):
        ...
        try:
            valor_actual = float(entry.get())
        except:
            valor_actual = 0.0
            
        nuevo_monto = simpledialog.askfloat(
            "Modificar Presupuesto", 
            f"Ingrese nuevo presupuesto para '{categoria}' en {mes}:", 
    ...
    def copiar_presupuesto_mes(self):
        ...
        def copiar():
            ...
            if mes_origen in self.presupuesto_mensual_por_mes:
                ...
                ventana.destroy()
                messagebox.showinfo("Éxito", f"Presupuesto copiado de {mes_origen} a {mes_destino}")
            else:
                messagebox.showwarning("Advertencia", f"No hay presupuesto en {mes_origen}")
        ...
        if messagebox.askyesno("Confirmar", "¿Aplicar este presupuesto a todos los meses?"):
            ...
            for categoria, entry in self.presupuesto_mensual_entries.items():
                try:
                    ...
                    messagebox.showerror("Error", f"Monto inválido en '{categoria}'")
                    return
                    
            for mes in self.meses: 
                self.presupuesto_mensual_por_mes[mes] = presupuesto_actual.copy()
            self.guardar_datos()
            self.registrar_actividad("Aplicar presupuesto a todos los meses")
    ...
    def mostrar_resumen_anual(self):
        ...
        text.insert('end', "\n" + "=" * 80 + "\n")
        text.insert('end', f"{'RESUMEN POR CATEGORÍA':^80}\n".center(80))
        text.insert('end', "=" * 80 + "\n\n")
        
        for categoria, total in sorted(total_anual_por_categoria.items(), key=lambda x: x[1], reverse=True):
            porcentaje = (total / total_anual * 100) if total_anual > 0 else 0
            text.insert('end', f"  {categoria:40s} ${total:>12,.2f} ({porcentaje:.1f}%)\n")
        
        text.insert('end', "\n" + "=" * 80 + "\n")
        text.insert('end', f"{'TOTAL ANUAL':40s} ${total_anual:>12,.2f}\n")
        text.insert('end', "=" * 80 + "\n")
        ...
        if self.puede_modificar():
            ...
            for cat, subcats in self.categorias_personalizadas.items():
                if cat in todas_cats:
                    todas_cats[cat] = list(set(todas_cats[cat] + subcats))
                else:
                    todas_cats[cat] = list(subcats)
            ...
            for categoria, subcategorias in sorted(todas_cats.items()):
                ...
                subcategorias_vars = self.proveedor_categorias_vars[categoria]['subcategorias']
                
                cb_principal = tk.Checkbutton(
                    frame_cat,
                ...
                cb_principal.pack(anchor='w')
                
                for subcat in sorted(subcategorias):
                    var_subcat = tk.BooleanVar()
                    subcategorias_vars[subcat] = var_subcat
                    

                    def make_callback(cat=categoria, sub=subcat):
                        return lambda: self.actualizar_estado_categoria_principal(cat)
                    
                    cb_subcat = tk.Checkbutton(
                        frame_cat,
                        text=f"  • {subcat}",
                        variable=var_subcat,
                        font=('Arial', 9),
                    ...
                    cb_subcat.pack(anchor='w', padx=20)
    ...
    def refrescar_categorias_proveedores(self):
        if not hasattr(self, 'frame_categorias_prov'):
            messagebox.showinfo("Info", "Primero abra la pestaña de proveedores.")
            return
        ...
        for categoria, subcategorias in sorted(todas_cats.items()):
            ...
            for subcat in sorted(subcategorias):
                ...
                cb_subcat = tk.Checkbutton(
                    frame_cat, text=f"  • {subcat}", variable=var_subcat, 
                    font=('Arial', 9),
                    command=make_callback()
                )
                cb_subcat.pack(anchor='w', padx=20)
    ...
    def toggle_todas_subcategorias(self, categoria):
        ...
        if categoria not in self.proveedor_categorias_vars:
            return
    ...
    def actualizar_estado_categoria_principal(self, categoria):
        """Actualiza el estado del checkbox principal basado en las subcategorías"""
        if not self.puede_modificar() or categoria not in self.proveedor_categorias_vars:
            return
            
        vars_dict = self.proveedor_categorias_vars[categoria]
        todas_seleccionadas = all(var.get() for var in vars_dict['subcategorias'].values())
        vars_dict['principal'].set(todas_seleccionadas)
    ...
    def cargar_proveedor_seleccionado(self, event):
        ...
        for categoria, subcats in categorias.items():
            if categoria in self.proveedor_categorias_vars:
                ...
                self.actualizar_estado_categoria_principal(categoria)
    ...
    def limpiar_form_proveedor(self):
        ...
        if hasattr(self, 'proveedor_categorias_vars'):
            for vars_dict in self.proveedor_categorias_vars.values():
                vars_dict['principal'].set(False)
                for var in vars_dict['subcategorias'].values():
                    var.set(False)
    ...
    def actualizar_lista_proveedores(self):
        if not hasattr(self, 'tree_proveedores'):
            return
    ...
    def crear_tab_registro(self):
        ...
        self.tree_gastos = ttk.Treeview(
            frame_tabla, 
            columns=('fecha', 'categoria', 'subcategoria', 'monto', 'proveedor', 'descripcion'), 
            show='headings', 
            yscrollcommand=scroll_y.set, 
    ...
    def actualizar_subcategorias(self, event=None):
        ...
        if categoria in todas_cats:
            ...
            for prov in self.proveedores.values():
                if categoria in prov.get('categorias', {}):
                    nombres_proveedores.append(prov['nombre'])
    ...
    def registrar_gasto(self):
        ...
        try:
            ...
            if not all([fecha, categoria, subcategoria, monto_str]):
                messagebox.showwarning("Advertencia", "Complete los campos obligatorios (fecha, categoría, subcategoría, monto)")
                return

            fecha_obj = self.validar_fecha(fecha)
            if not fecha_obj:
                messagebox.showerror("Error", "Fecha inválida. Use formato YYYY-MM-DD")
                return

            valido, monto, mensaje_error = self.validar_monto(monto_str)
            if not valido:
            ...
            if mes in self.presupuesto_mensual_por_mes:
                ...
                gastos_categoria_mes = sum(
                    ...
                    self.validar_fecha(t['fecha']) and
                    self.validar_fecha(t['fecha']).month == fecha_obj.month and 
                    self.validar_fecha(t['fecha']).year == fecha_obj.year
                )
                ...
                if monto > disponible:
                    ...
                    mensaje = (
                        f"⚠️ ADVERTENCIA: Presupuesto insuficiente\n"
            ...
            self.guardar_datos()
            self.registrar_actividad("Registro de gasto", f"${monto:,.2f} en {categoria} - {subcategoria}")
            self.actualizar_tabla_gastos()
            self.limpiar_campos_gasto()
            messagebox.showinfo("Éxito", "Gasto registrado correctamente")

        except Exception as e:
            messagebox.showerror("Error", f"Error al registrar el gasto: {str(e)}")
            self.logger.error(f"Error en registro de gasto: {e}")
    ...
    def ventana_origen_presupuesto(self, mes_destino, categoria_destino, monto_necesario):
        ...
        def actualizar_disponible():
            ...
            if mes_sel and cat_sel:
                ...
                mes_idx_origen = self.meses.index(mes_sel) + 1
                gastado = sum(
                    t['monto'] for t in self.transacciones
                    if t.get('categoria') == cat_sel and 
                    self.validar_fecha(t['fecha']) and
                    self.validar_fecha(t['fecha']).month == mes_idx_origen and
                    self.validar_fecha(t['fecha']).year == self.año_actual
                )
        ...
        color = '#4CAF50' if sobrante_disponible >= monto_necesario else '#f44336'
        icono = "✅" if sobrante_disponible >= monto_necesario else "❌"
        ...
        if messagebox.askyesno("Confirmar", "¿Está seguro de eliminar este gasto?"):
            ...
            for i, t in enumerate(self.transacciones):
                if (t['fecha'] == valores[0] and 
                    ...
                    abs(t['monto'] - float(valores[3].replace('$', '').replace(',', ''))) < 0.01):
                    del self.transacciones[i]
                    break
            self.guardar_datos()
            self.actualizar_tabla_gastos()
            self.registrar_actividad("Eliminar gasto", f"Monto: {valores[3]}")
            messagebox.showinfo("Éxito", "Gasto eliminado correctamente")

    def actualizar_tabla_gastos(self):
        if not hasattr(self, 'tree_gastos'):
            return
    ...
    def crear_tab_control(self):
        ...
        self.combo_mes_control.bind('<<ComboboxSelected>>', lambda e: self.actualizar_control_mensual())
        ...
        self.tree_control = ttk.Treeview(
            frame_detalle, 
            columns=('categoria', 'presupuesto', 'gastado', 'disponible', 'porcentaje', 'estado'), 
            show='headings', 
            yscrollcommand=scroll_y.set
        )
    ...
    def actualizar_control_mensual(self):
        if not hasattr(self, 'tree_control'):
            return
        ...
        if mes_seleccionado in self.presupuesto_mensual_por_mes:
            for categoria, presupuesto in self.presupuesto_mensual_por_mes[mes_seleccionado].items():
                gastos = sum(
                    ...
                    if t['categoria'] == categoria and 
                    self.validar_fecha(t['fecha']) and
                    self.validar_fecha(t['fecha']).month == mes_numero and 
                    self.validar_fecha(t['fecha']).year == self.año_actual
                )
                ...
                if presupuesto == 0:
                    ...
                    estado = f"EXCEDIDO (${abs(disponible):,.2f})"
                    tag = 'danger'
                elif porcentaje <= 75:
                    estado = "OK"
                    tag = 'ok'
                ...
                if (mes_seleccionado in self.presupuesto_modificado and 
                    categoria in self.presupuesto_modificado.get(mes_seleccionado, {}) and 
                    self.presupuesto_modificado[mes_seleccionado].get(categoria, False)):
                    estado += " (MOD)"
                    tag = 'modified'
        ...
        if disponible < 0:
            self.label_disponible_mes.config(text=f"-${abs(disponible):,.2f}")
            self.label_disponible_mes.master.config(bg='#f44336')
        else:
            self.label_disponible_mes.config(text=f"${disponible:,.2f}")
            self.label_disponible_mes.master.config(bg='#2196F3')
    ...
    def crear_tab_sobrantes(self):
        ...
        self.tree_sobrantes = ttk.Treeview(
            frame_tabla_sobrantes, 
            columns=('categoria', 'presupuesto', 'gastado', 'sobrante'), 
            show='headings', 
        ...
        if self.puede_modificar():
            ...
            self.combo_categoria_destino = ttk.Combobox(frame_transferencia, values=todas_cats_sob, state='readonly', width=25)
    ...
    def actualizar_sobrantes_disponibles(self, event=None):
        if not hasattr(self, 'tree_sobrantes'):
            return
    ...
    def realizar_transferencia(self):
        ...
        try:
            ...
            monto_str = self.entry_monto_transferir.get()
            tipo = self.tipo_transferencia.get()
            
            if not all([mes_origen, categoria_origen, monto_str]): 
                messagebox.showwarning("Advertencia", "Complete todos los campos")
                return
            ...
            presupuesto = self.presupuesto_mensual_por_mes.get(mes_origen, {}).get(categoria_origen, 0)
            gastos = sum(
                t['monto'] for t in self.transacciones 
            ...
            if monto > disponible: 
                messagebox.showerror("Error", f"No hay suficiente sobrante. Disponible: ${disponible:,.2f}")
                return
            ...
            if tipo == "mes":
                ...
                if mes_destino not in self.presupuesto_mensual_por_mes: 
                    self.presupuesto_mensual_por_mes[mes_destino] = {}
                self.presupuesto_mensual_por_mes[mes_destino][categoria_destino] = \
                    self.presupuesto_mensual_por_mes[mes_destino].get(categoria_destino, 0) + monto
                if mes_destino not in self.presupuesto_modificado: 
                    self.presupuesto_modificado[mes_destino] = {}
                self.presupuesto_modificado[mes_destino][categoria_destino] = True
                mensaje = f"Transferencia: ${monto:,.2f} de {categoria_origen} en {mes_origen} → A {mes_destino}"
            else:
                ...
                if not categoria_destino: 
                    ...
                    return
                self.presupuesto_mensual_por_mes[mes_origen][categoria_destino] = \
                    self.presupuesto_mensual_por_mes[mes_origen].get(categoria_destino, 0) + monto
                if mes_origen not in self.presupuesto_modificado: 
                    self.presupuesto_modificado[mes_origen] = {}
    ...
    def generar_graficos(self):
        ...
        if mes_seleccionado not in self.presupuesto_mensual_por_mes: 
            messagebox.showinfo("Sin Datos", "No hay presupuesto configurado para este mes")
            return
        ...
        for categoria, presupuesto in self.presupuesto_mensual_por_mes[mes_seleccionado].items():
            ...
            categorias.append(categoria[:20] + '..' if len(categoria) > 20 else categoria)
        ...
        ax1.set_title(f'Presupuesto vs Gastado - {mes_seleccionado} {self.año_actual}')
        ax1.set_xticks(x)
        ...
        for i in range(6, 0, -1):
            mes_idx = mes_numero - i
            if mes_idx < 1:
                mes_idx += 12
            mes_nombre = self.meses[mes_idx - 1]
            total = sum(
                t['monto'] for t in self.transacciones 
                if self.validar_fecha(t['fecha']) and
                self.validar_fecha(t['fecha']).month == mes_idx and
                self.validar_fecha(t['fecha']).year == self.año_actual
            )
            meses_anteriores.append(mes_nombre[:3])
            totales_gastados.append(total)

        ax4.plot(meses_anteriores, totales_gastados, marker='o', linewidth=2, markersize=8, color='#2196F3')
        ax4.set_xlabel('Mes')
        ax4.set_ylabel('Total Gastado ($)')
        ax4.set_title(f'Tendencia de Gasto (Últimos 6 Meses) - {self.año_actual}')
    ...
    def cargar_datos(self):
        ...
        datos = self.seguridad.cargar_archivo_cifrado(ARCHIVOS['datos'])
        if datos:
            self.transacciones = datos
        else:
            self.transacciones = []
        
        datos = self.seguridad.cargar_archivo_cifrado(ARCHIVOS['presupuesto_mensual'])
        if datos:
            self.presupuesto_mensual_por_mes = datos.get('presupuesto', {})
            self.presupuesto_modificado = datos.get('modificado', {})
        else:
            self.presupuesto_mensual_por_mes = {}
            self.presupuesto_modificado = {}
        
        datos = self.seguridad.cargar_archivo_cifrado(ARCHIVOS['proveedores'])
        if datos:
        ...
        datos = self.seguridad.cargar_archivo_cifrado(ARCHIVOS['presupuestos_anuales'])
        if datos:
            self.presupuestos_por_año = datos
        else:
            self.presupuestos_por_año = {}
        
        datos = self.seguridad.cargar_archivo_cifrado(ARCHIVOS['sobrantes_anuales'])
        if datos:
            self.sobrantes_anuales = datos
        else:
            self.sobrantes_anuales = {}
        
        datos = self.seguridad.cargar_archivo_cifrado(ARCHIVOS['categorias_personalizadas'])
        if datos:
            self.categorias_personalizadas = datos
        else:
            self.categorias_personalizadas = {}
    ...
    def migrar_transacciones_antiguas(self):
        """Migra transacciones antiguas añadiendo campos faltantes"""
        modificado = False
    ...
    def guardar_datos(self):
        ...
        self.seguridad.guardar_archivo_cifrado(ARCHIVOS['datos'], self.transacciones)
        
        datos_presupuesto = {
            'presupuesto': self.presupuesto_mensual_por_mes,
            'modificado': self.presupuesto_modificado
        }
        self.seguridad.guardar_archivo_cifrado(ARCHIVOS['presupuesto_mensual'], datos_presupuesto)
        
        self.seguridad.guardar_archivo_cifrado(ARCHIVOS['proveedores'], self.proveedores)
    ...
    def guardar_presupuestos_anuales(self):
        self.seguridad.guardar_archivo_cifrado(ARCHIVOS['presupuestos_anuales'], self.presupuestos_por_año)

    def guardar_sobrantes_anuales(self):
        self.seguridad.guardar_archivo_cifrado(ARCHIVOS['sobrantes_anuales'], self.sobrantes_anuales)

    def guardar_categorias_personalizadas(self):
        self.seguridad.guardar_archivo_cifrado(ARCHIVOS['categorias_personalizadas'], self.categorias_personalizadas)
...
def main():
    ...
    def on_closing():
        if messagebox.askyesno("Salir", "¿Está seguro de salir del sistema?"):
            app.registrar_actividad("Cierre de sesión")
            root.destroy()
